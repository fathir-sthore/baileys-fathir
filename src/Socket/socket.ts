...
export const makeSocket = (config: SocketConfig) => {
	...
	const requestPairingCode = async (phoneNumber: string, customPairingCode?: string): Promise<string> => {
		// ── Node 23+ compat: randomBytes tersedia via native crypto, pastikan buffer valid ──
		const rawBytes = randomBytes(5)
		const fallbackCode = bytesToCrockford(Buffer.isBuffer(rawBytes) ? rawBytes : Buffer.from(rawBytes))

		// Custom pairing code: validasi & normalisasi (uppercase, strip non-alphanum)
		let pairingCode: string
		if (customPairingCode) {
			const normalized = customPairingCode.toUpperCase().replace(/[^A-Z0-9]/g, '')
			if (normalized.length !== 8) {
				throw new Error(
					`Custom pairing code harus tepat 8 karakter alfanumerik (diberikan: "${customPairingCode}", setelah normalisasi: "${normalized}")`
				)
			}
		...
		logger.info(
			{ phoneNumber, pairingCode, nodeVersion: process.versions.node },
